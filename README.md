...
もしコマンドが見つからない場合は、まずモジュール実行を試してください。

```powershell
python -m digitone_syx_toolkit gui
```

これは `digitone_syx_toolkit` の console script が PowerShell の `PATH` から見えていない場合の回避策です。
...
インストール済みでも `digitone_syx_toolkit` が見つからない場合は、使用中 Python の `Scripts` ディレクトリを `PATH` に追加してください。

例:

```powershell
$env:Path += ";C:\Users\<your-user>\AppData\Local\Python\pythoncore-3.14-64\Scripts"
digitone_syx_toolkit gui
```
